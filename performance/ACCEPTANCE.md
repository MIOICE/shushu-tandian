...
| 验收项 | 负载 | 实测结果 | 结论 |
|---|---:|---:|---|
| 秒杀受理 | 10 线程 × 100 次，共 1000 次 | 平均 9ms，最小 3ms，最大 162ms，HTTP 错误 0 | 通过 |
...
| 两级缓存 | 2000 次/分钟 | 命中 1999，回源 1，命中率 99.95%，HTTP 错误 0 | 通过 |
| 三维限流 | 同一用户/IP/设备 1000 次 | 放行 30，HTTP 429 共 970，拦截率 97% | 通过 |
| 超时关单 | 10 个待支付订单超过 15 分钟 | 10 个订单关闭，MySQL 与 Redis 库存均回补 | 通过 |

## 秒杀数据库核验

最终验收活动初始库存为 10。MQ 消费完成后立即核验并将 10 个订单置为已支付，以免正常的 15 分钟超时任务改变库存快照：
...
```json
{"requests":2000,"localHits":1997,"redisHits":2,"databaseQueries":1,"hitRate":0.9995}
```
